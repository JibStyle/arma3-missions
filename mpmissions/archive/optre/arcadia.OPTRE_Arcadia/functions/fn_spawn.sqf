...
my_spawn_register_waypoint = {
    ...
    _logic setVariable ["my_spawn_wp_statements", _statements];
...
my_spawn_activate_trigger = {
    ...
    for "_i" from 0 to (_n - 1) do {
        ...
        sleep my_spawn_delay;
    };
};

my_spawn_deactivate_trigger = {
    params ["_trigger"];
    if (!isServer) exitWith {};
    private _emitters = synchronizedObjects _trigger select {
        _x call BIS_fnc_objectType params ["_category", "_type"];
        _category == "Logic" && _type == "Logic"
            && _x getVariable ["my_spawn_type", ""] == "emitter";
    };
    _emitters apply {
        private _emitter = _x;
        private _triggers = synchronizedObjects _emitter select {
            _x isKindOf "EmptyDetector"
        };
        _triggers apply {_x synchronizeObjectsRemove [_emitter]};
    };
};
...
my_spawn_activate_emitter = {
    ...
    _vehicles = _vehicles apply {_x # 0};
    _vehicles apply {[_x] call _setDir};
    ...
    _tr_vehicles = _tr_vehicles apply {_x # 0};
    _tr_vehicles apply {[_x] call _setDir};
    ...
    _tr_vehicles apply {_x allowDamage true};
    if (my_spawn_debug) then {
        allCurators apply {
            _x addCuratorEditableObjects [
                _units + _vehicles + _tr_units + _tr_vehicles, true
            ];
        };
    };
};
...
my_spawn_choose_waypoints = {
    ...
    private _transform = {[
        ...
        _x getVariable ["my_spawn_wp_statements", ["true", ""]]
    ]};
