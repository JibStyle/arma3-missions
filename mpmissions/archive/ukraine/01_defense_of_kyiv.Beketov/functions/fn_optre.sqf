...
my_optre_pelicanLoad = {
    ...
    if (_loaded) then {
        _pelican setVariable ["my_optre_pelicanCargo", _cargo, true];
    };

    _loaded;
};

my_optre_pelicanUnload = {
    params ["_pelican"];
    if (!isServer) exitWith {};
    private _cargo =
        _pelican getVariable ["my_optre_pelicanCargo", objNull];
    ...
    [_pelican, _cargo] spawn {
        ...
        _pelican setVariable ["my_optre_pelicanCargo", objNull, true];
    };
};

publicVariable "my_optre_pelicanLoad";
publicVariable "my_optre_pelicanUnload";
